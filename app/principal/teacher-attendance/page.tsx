...
import { Calendar, Clock, TrendingUp, UserCheck, Filter } from "lucide-react";
import { Badge } from "@/components/ui/badge";
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from "@/components/ui/select";
import { getPrincipalTeacherAttendance } from "@/lib/fastapi-client";
...
export default function TeacherAttendancePage() {
  ...
  useEffect(() => {
    ...
    if (status === "authenticated" && session?.user?.role !== "principal" && session?.user?.role !== "admin") {
      redirect("/");
    }
  }, [status, session]);
  ...
  const fetchAttendance = async () => {
    try {
      setLoading(true);
      const data = await getPrincipalTeacherAttendance(selectedDate) as any;
